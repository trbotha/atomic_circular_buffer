...
public:
	...
	std::atomic<size_t> debug_reads = 0;
	std::atomic<size_t> debug_writes = 0;

private:
	constexpr static std::size_t m_size = 1024; //power of 2 is best
	std::array<T, m_size> m_buffer;
	//T m_buffer[m_size];
	std::atomic<size_t> m_write_ptr = 0;
	std::atomic<size_t> m_read_ptr = 0;
	std::atomic<size_t> m_space = m_size;
